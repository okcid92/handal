...
export function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadOverview() {
      try {
        ...
        if (mounted) {
          setPendingMessage(error instanceof Error ? error.message : "Impossible de charger le profil");
        }
      } finally {
        if (mounted) {
  ...
  async function proposeTheme(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiFetch<{ theme: { id: string; status: string } }>("/api/themes/propose", {
        method: "POST",
        body: JSON.stringify({
          title: themeTitle,
      ...
      setThemeMessage(`Theme créé: ${result.theme.id} (${result.theme.status})`);
      setThemeTitle("");
      setThemeDescription("");
    } catch (error) {
      setThemeMessage(error instanceof Error ? error.message : "Erreur proposition theme");
    }
  }
  ...
  async function uploadDocument(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiFetch<{ document: { id: string } }>("/api/documents/upload", {
        method: "POST",
        body: JSON.stringify({
      ...
      setDocumentMessage(error instanceof Error ? error.message : "Erreur dépôt document");
    }
  }
  ...
  async function runAutoTest(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiFetch<{ autoTest: unknown }>(`/api/documents/${autoTestDocumentId}/auto-test`, {
        method: "POST",
      });
      ...
      setAutoTestMessage(error instanceof Error ? error.message : "Erreur auto-test");
    }
  }

  if (pendingLoading) {
    return <DashboardShellLoading label="Chargement du tableau de bord étudiant..." />;
  }
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        <Panel title="Proposer un thème" accent="amber">
          <form className="space-y-4" onSubmit={proposeTheme}>
            <Input label="Titre" value={themeTitle} onChange={setThemeTitle} placeholder="Détection de plagiat multilingue" />
            <Textarea label="Description" value={themeDescription} onChange={setThemeDescription} placeholder="Décrire le sujet et le périmètre." />
        ...
        <Panel title="Dépôt du mémoire" accent="violet">
          <form className="space-y-4" onSubmit={uploadDocument}>
            <Input label="Theme ID" value={documentThemeId} onChange={setDocumentThemeId} placeholder="12" />
            <Input label="Nom du fichier" value={documentOriginalName} onChange={setDocumentOriginalName} placeholder="memoire-final.pdf" />
            <Input label="MIME type" value={documentMimeType} onChange={setDocumentMimeType} placeholder="application/pdf" />
            <Input label="Taille fichier" type="number" value={String(documentFileSize)} onChange={(value) => setDocumentFileSize(Number(value || 0))} placeholder="1024" />
            <Input label="Checksum" value={documentChecksum} onChange={setDocumentChecksum} placeholder="sha256:..." />
        ...
        <Panel title="Auto-test" accent="rose">
          <form className="space-y-4" onSubmit={runAutoTest}>
            <Input label="Document ID" value={autoTestDocumentId} onChange={setAutoTestDocumentId} placeholder="25" />
...
function DashboardShellLoading({ label }: { label: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-sm text-zinc-300">{label}</div>;
}

function Panel({ title, accent, children }: { title: string; accent: "emerald" | "cyan" | "amber" | "violet" | "rose"; children: React.ReactNode }) {
  ...
  return (
    <section className="rounded-[1.75rem] border border-white/10 bg-white/5 p-5 backdrop-blur">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className={`rounded-full px-3 py-1 text-xs font-medium uppercase tracking-[0.2em] ${accentClasses[accent]}`}>
          Live
        </span>
      </div>
      {children}
...
function SubmitButton({ children }: { children: React.ReactNode }) {
  return (
    <button type="submit" className="inline-flex w-full items-center justify-center rounded-2xl bg-white px-4 py-3 font-semibold text-zinc-950 transition hover:bg-zinc-200">
      {children}
    </button>
  );
}
...
function Message({ value }: { value: string }) {
  return <div className="mt-4 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">{value}</div>;
}
